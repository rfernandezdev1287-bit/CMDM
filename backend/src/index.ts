...
// 2. Inicialización del Servidor de Sockets (DDD: Infrastructure)
const server = new SocketServer();
server.start(PORT);

// 3. Inicialización de la Vigilancia (DDD: Application Service)
const defaultPath = path.join(__dirname, '..', '..');
const watchPath = process.env.VIGILANCIA_PATH || defaultPath;

const intercessionService = new IntercessionService(watchPath, server.getIO());
